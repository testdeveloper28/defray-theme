...
namespace Dotsquares\Marketplace\Model\Cssconfig;
...
class Generator
{
    ...
    protected function generateStoreCss($type, $storeId) {
        ...
        $str1 = '_'.$storeCode;
        $str2 = $type.$str1.'.css';
        $str3 = $this->getCssConfigDir().$str2;
		$str4 = 'marketplace/css/theme-'.$type.'.phtml';
		$this->_coreRegistry->register('cssgen_marketplace', $storeCode);

        try {
            $block = $this->_layoutManager->createBlock('Dotsquares\Marketplace\Block\Template')->setData('area','frontend')->setTemplate($str4)->toHtml();
        ...
        $this->_coreRegistry->unregister('cssgen_marketplace');
    }
	
	public function getCssConfigDir(){
		$base = BP;
        $generatedCssFolder = 'marketplace/configed_css/';
        $generatedCssPath = 'pub/media/'.$generatedCssFolder;
        $generatedCssDir = $base.'/'.$generatedCssPath;
